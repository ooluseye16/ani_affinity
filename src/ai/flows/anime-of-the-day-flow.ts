...
const AnimeOfTheDayOutputSchema = z.object({
  ...
  reason: z
    .string()
    .describe(
      'A compelling 1-2 sentence reason explaining why this anime is chosen as the "Anime of the Day" for today.'
    ),
...
export async function getAnimeOfTheDay(): Promise<AnimeOfTheDayOutput> {
  return animeOfTheDayFlow();
}

const animeOfTheDayPrompt = ai.definePrompt({
  name: 'animeOfTheDayPrompt',
  output: {schema: AnimeOfTheDayOutputSchema},
  prompt: `You are an AI curator with a deep knowledge of anime. Your task is to select and present one "Anime of the Day".
  This anime should be noteworthy, critically acclaimed, a hidden gem, or particularly relevant for some reason.
  ...
  5. Reason: A compelling 1-2 sentence explanation of why this specific anime is chosen as the "Anime of the Day" for today. Highlight what makes it special.
...
const animeOfTheDayFlow = ai.defineFlow(
  ...
  async () => {
    const {output} = await animeOfTheDayPrompt({});
    return output!;
  }
);
